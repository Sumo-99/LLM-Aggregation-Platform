...
class PodCreationRequest(BaseModel):
    app_name: str
    ...
    def validate_app_name(cls, v):
        ...
        # Ensure the app name only contains valid characters
        if not v.replace('-', '').replace('_', '').isalnum():
            raise ValueError('App name can only contain alphanumeric characters, hyphens, and underscores')
        return v

@router.post("/trigger_pod_creation")
async def create_pod(request: PodCreationRequest):
    try:
        # Constants
        ...
        try:
            print(f"Generating application files for {request.app_name}...")
            generate_files(request.app_name, REQUIREMENTS, folder_path)

            print("Building and pushing Docker image...")
            image_uri = build_and_push_image(request.app_name, ECR_REPOSITORY, folder_path)

            print("Deploying to Kubernetes...")
            deploy_to_k8s(request.app_name, image_uri)

            print("Application deployed successfully!")
            
            return {
                "status": "success",
                "message": f"Application {request.app_name} deployed successfully",
                "image_uri": image_uri
            }
